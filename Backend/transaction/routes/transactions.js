...
const engine = {
    placeOrder: async (order) => {
        try {
            const response = await axios.post(`${MatchingEngine}/placeOrder`, order);
            return response.data;
        } catch (error) {
            console.error("Error communicating with Matching Engine:", error.message);
            throw error;
        }
    },
    getOrderBook: async () => {
        try {
            const response = await axios.get(`${MatchingEngine}/getOrderBook`);
            return response.data;
        } catch (error) {
            console.error("Error fetching order book from Matching Engine:", error.message);
            throw error;
        }
    }
};
